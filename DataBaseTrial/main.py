...
def update_value(occupied, seat_id):
    connection = sqlite3.connect("cinema.db")
    # connection.execute(""" UPDATE "Seat" SET "taken" = 1 WHERE "seat_id" = "A3" """)  NORMAL UPDATE WITHOUT PARAMS
    connection.execute(""" UPDATE "Seat" SET "taken" = ? WHERE "seat_id" = ? """, [occupied, seat_id])
    connection.commit()
    connection.close()
...
update_value(occupied=1, seat_id="A1")